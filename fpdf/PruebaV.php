...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $queryc = "SELECT * FROM paciente
        CROSS JOIN factura 
        where paciente.cedula = factura.id_paciente order by id_factura DESC limit 1;";
        $resultc = mysqli_query($conn, $queryc);
        $dato = $resultc->fetch_object();

                $querycc = "SELECT * FROM paciente
        CROSS JOIN paciente_vs_correo ON paciente.cedula = paciente_vs_correo.id_paciente
        CROSS JOIN factura
        CROSS JOIN correo
        where paciente.cedula = factura.id_paciente AND correo.id_correo = paciente_vs_correo.id_correo  order by id_factura DESC limit 1;";
        $resultcc = mysqli_query($conn, $querycc);
        $datoc = $resultcc->fetch_object();
        
//        $querycc = "SELECT nombre FROM paciente WHERE cedula=$datoc.id_paciente";
//        $resultcc = mysqli_query($conn, $querycc);
//        $dato = $resultc->fetch_object();
        ...
        $this->Cell(96, 10, utf8_decode("Ubicación : Santiago De Los Caballeros, Santiago, Dominican Republic. "), 0, 0, 'L', 0);
        $this->Ln(5);
        /* TELEFONO */
        $this->SetFont('Arial', 'B', 10);
        $this->SetTextColor(39, 39, 51);
        ...
        $this->SetFont('Arial', 'B', 10);
        $this->SetTextColor(39, 39, 51);
        $this->Cell(20, 10, utf8_decode("Paciente : ". $dato->nombre), 0, 0, 'C', 0);
        $this->Ln(5);

        /* TELEFONO */
//        $this->SetFont('Arial', '', 10);
//        $this->SetTextColor(39, 39, 51);
//        $this->Cell(200, 3, utf8_decode("Teléfono : ".$dato->telefono), 0, 0, 'C', 0);

        $this->Ln(5);
        /* COREEO */
        $this->SetFont('Arial', '', 10);
        $this->SetTextColor(39, 39, 51);
        $this->Cell(20, 10, utf8_decode("Correo : ".$datoc->correo), 0, 0, 'C', 0);
        $this->Ln(5);
        /* CEUDLA */
        $this->SetFont('Arial', 'B', 10);
        $this->SetTextColor(39, 39, 51);
        $this->Cell(182, 0, utf8_decode("Cedula: ".$dato->cedula), 0, 0, 'C', 0);
        $this->Ln(10);
        /* TITULO DE LA TABLA */
        //color
...
$query = "SELECT * FROM detalle_factura order by id_factura DESC limit 1";
$result = mysqli_query($conn, $query);
/* $consulta_reporte_alquiler = $conexion->query("  "); */

while ($row = $result->fetch_assoc()) {
    $pdf->Cell(18, 10, $row['id_factura'], 1, 0, 'C', 0);
    $pdf->Cell(40, 10, $row['id_proc'], 1, 0, 'C', 0);
    $pdf->Cell(30, 10, $row['precio'], 1, 0, 'C', 0);
    $pdf->Cell(40, 10, $row['cobertura'], 1, 0, 'C', 0);
    $pdf->Cell(40, 10, $row['cantidad'], 1, 1, 'C', 0);
    // $pdf->Cell(25, 10, $row('ESTADO'), 1, 1, 'C', 1);
}
